...
import { checkGlobal } from '../../functions/common';
        ...
        <React.Fragment>
            {
                checkNativeShare() !== undefined ?
                    <button className={Styles.ShareBtn} title='Share article via your applications' onClick={() => nativeShare(url, title)}>
                        <Share /> Share
                    </button>
                :
                    <p style={{fontSize: '30px', margin: '20px 0'}}>
                        <a href={`https://twitter.com/intent/tweet?text=${title} by @iamdillion - https://dillionmegida.com${url}`} >
